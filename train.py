...
def user_based(userId, algo, n=5):
...
md = pd. read_csv('movies_metadata.csv',low_memory=False).copy()
...
links_small = pd.read_csv('links_small.csv',low_memory=False).copy()
...
credits = pd.read_csv('credits.csv',low_memory=False).copy()
keywords = pd.read_csv('keywords.csv',low_memory=False).copy()
...
titles = smd['title']
reader = Reader()
ratings = pd.read_csv('ratings_small.csv',low_memory=False).copy()
...
id_map = pd.read_csv('links_small.csv',low_memory=False)[['movieId', 'tmdbId']].copy()
id_map['tmdbId'] = id_map['tmdbId'].apply(convert_int)
id_map.columns = ['movieId', 'id']
id_map.dropna().astype(int)
id_map = id_map.merge(smd[['title', 'id']], on='id',how="left")
indices_map=id_map.set_index('movieId')[['title']]
id_map = id_map.reset_index().set_index('title')
